...
namespace TSWebServices
{
    ...
    public class TicketService : System.Web.Services.WebService
    {
      ...
      ";
        ...
        {
            TeamSupport.Data.Group group = groupID != null ? Groups.GetGroup(TSAuthentication.GetLoginUser(), (int)groupID) : null;
...
";
        ...
        public int[] NewTicket(string data)
        {
            ...
            //// new action - future
            //if (TeamSupport.IDTree.ConnectionContext.IsEnabled)  // new action - new ticket
            //{
                ...
                {                  
                    ...
                    {                                             
                        ...
                        if (crmLinkTable != null && !string.IsNullOrEmpty(crmFieldName))
                        {
                            ...
                            var assignedUserToTicketId = ticket.UserID; //Business logic: UserID is the assigned person of the ticket; CreatorID is whomever originally created the ticket.
                            if (assignedUserToTicketId != null)
                            {
                                var userMetaData = jiraService.GetUserMetaData(assignedUserToTicketId);
                                var jiraUserMetaData = jiraService.GetUsers(crmLinkTable.HostName, crmLinkTable.Username, crmLinkTable.Password);
                                assignee = jiraUserMetaData.Where(a => a.emailAddress == userMetaData.Email).FirstOrDefault();
                            }
                            else
                            {
                            ...
                            var jiraResponse = jiraService.CreateNewJiraTicket(crmLinkTable.HostName, crmLinkTable.Username, crmLinkTable.Password, crmLinkTable.DefaultProject, crmFieldName, issueFields);

                            var ticketLinkToJiraModel = CreateTicketLinkToJiraModel(loginUser.UserID, jiraService, ticket.TicketID, crmLinkId, jiraResponse, crmLinkTable.HostName);
        ...
        private TeamSupport.EFData.Models.TicketLinkToJira CreateTicketLinkToJiraModel(int userId, JiraService jiraService, int ticketId, int? crmLinkId, Issue jiraResponse, string hostName)
        {
            var ticketLinkToJiraAdd = new TeamSupport.EFData.Models.TicketLinkToJira();
            ticketLinkToJiraAdd.JiraKey = jiraResponse.key;
            ticketLinkToJiraAdd.JiraLinkURL = hostName + "/browse/" + jiraResponse.key;
            ...
            ticketLinkToJiraAdd.DateModifiedByJiraSync = DateTime.Now;
